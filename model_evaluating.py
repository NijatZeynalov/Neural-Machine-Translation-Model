...
# Load datasets
dataset = load_dataset('english-azerbaijani-both.pkl')
train = load_dataset('english-azerbaijani-train.pkl')
test = load_dataset('english-azerbaijani-test.pkl')
...
# prepare azerbaijani tokenizer
aze_tokenizer = create_tokenizer(dataset[:, 1])
aze_vocab_size = len(aze_tokenizer.word_index) + 1
aze_length = max_length(dataset[:, 1])

# Prepare data
trainX = encode_sequences(aze_tokenizer, aze_length, train[:, 1])
testX = encode_sequences(aze_tokenizer, aze_length, test[:, 1])
